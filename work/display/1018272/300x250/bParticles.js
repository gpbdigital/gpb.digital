...
function genParticles(options, callback){
  // Setup Canvas
  var container = document.getElementById(options.container);
  if (!container){
      return console.error('Cannot find container element: ' + options.container);
  }
  options.canvas = {};
...
// PRIMARY FUNCTION - loads the options file, then execute particle generation
function bParticles(optionsFile, container, callback){
      ...
      genParticles(options, function(timerID){
          if (callback){
            callback(timerID);
          }
      });
